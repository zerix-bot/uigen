...
import { User, Bot, Loader2, FileCode2, FolderOpen } from "lucide-react";
import { MarkdownRenderer } from "./MarkdownRenderer";

const toolLabels: Record<string, { done: string; loading: string; icon: React.ElementType }> = {
  str_replace_editor: { done: "Componente criado", loading: "Criando componente...", icon: FileCode2 },
  file_manager:       { done: "Arquivos organizados", loading: "Organizando arquivos...", icon: FolderOpen },
};
                          ...
                          case "reasoning":
                            return (
                              <div key={partIndex} className="mt-3 p-3 bg-white/50 rounded-md border border-neutral-200">
                                <span className="text-xs font-medium text-neutral-600 block mb-1">Raciocínio</span>
                                <span className="text-sm text-neutral-700">{part.reasoning}</span>
                              </div>
                            );
                          case "tool-invocation": {
                            const tool = part.toolInvocation;
                            const meta = toolLabels[tool.toolName];
                            const ToolIcon = meta?.icon ?? FileCode2;
                            ...
                            return (
                              <div key={partIndex} className="inline-flex items-center gap-2 mt-2 px-3 py-1.5 bg-neutral-50 rounded-lg text-xs border border-neutral-200">
                                {isDone ? (
                                  <>
                                    <ToolIcon className="w-3.5 h-3.5 text-emerald-500" />
                                    <span className="text-neutral-600">{meta?.done ?? tool.toolName}</span>
                                  </>
                                ) : (
                                  <>
                                    <Loader2 className="w-3.5 h-3.5 animate-spin text-blue-500" />
                                    <span className="text-neutral-600">{meta?.loading ?? tool.toolName}</span>
                                  </>
                                )}
                              </div>
                            );
                          ...
                          case "source":
                            return (
                              <div key={partIndex} className="mt-2 text-xs text-neutral-500">
                                Fonte: {JSON.stringify(part.source)}
                              </div>
                            );
                          case "step-start":
